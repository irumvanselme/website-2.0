...
const CardContainer = ({number}: { number: number }) => {
    return (
        <div className='m-3 bg-white text-black text-[10px] flex justify-center items-center h-3.5'>{number}</div>
    )
}
...
export default function Hero() {
    useEffect(() => {
        ...
        document.addEventListener("mousemove", (e) => {
            let [x, y] = [2350 / 2 - (e.clientY * 2350 / window.innerHeight), 2820 / 2 - (e.clientX * 2820 / window.innerWidth)];
            // console.clear();
